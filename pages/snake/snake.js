...
Page({
   data:{
        ...
        call: '',
        resultText:'',
        src: 'http://lihua1108.com/game/music/1.mp3'
   } ,
   onLoad:function(){
        ...
        if(snakeHEAD[0]<0||snakeHEAD[0]>=this.data.rows||snakeHEAD[1]>=this.data.cols||snakeHEAD[1]<0){
                clearInterval(this.data.timer);
                    ...
                    if (this.data.score == 0) {
                      this.setData({
                        call: '单身狗',
                        resultText: '如果你是唐僧,我会把你送到女儿国.'
                      })
                    }
                    else if(this.data.score>0 && this.data.score<=50){
                      this.setData({
                        call: '小学生',
                        resultText: '班主任最经典的一句话:整栋楼里就咱们班最吵'
                      })
                    }
                    else if(this.data.score> 50 && this.data.score<= 100){
                      this.setData({
                        call: '行云流水',
                        resultText: '穿别人的鞋,走自己的路,让他们找去吧.'
                      })
                    }
                    else if(this.data.score>100 && this.data.score<= 150){
                      this.setData({
                        call: '卷土重来',
                        resultText: '人生一世,浮华若梦，总有一人,视你如命.'
                      })
                    }
                    else if(this.data.score> 150 && this.data.score<=200){
                        this.setData({
                          call: '风流人物',
                          resultText: '一失足成千古风流人物.'
                        })
                    }
                    else if(this.data.score>200 && this.data.sore<=300){
                        this.setData({
                          call: '你是英雄',
                          resultText: '英雄不问出路,流氓不看岁数!'
                        })
                    }else{
                      this.setData({
                        call: '白山羊老铁',
                        resultText: '陌上花开,可缓缓归矣。'
                      })
                    }
                    this.audioCtx.pause()
        }
